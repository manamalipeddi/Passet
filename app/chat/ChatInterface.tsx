...
export default function ChatInterface({ initialMessages }: { initialMessages: Message[] }) {
  ...
  async function addWord(word: string, msgIdx: number) {
    ...
    try {
      ...
      if (addData.already_exists) {
        setWordStatus(p => ({ ...p, [key]: 'exists' }));
        return;
      }
  ...
  return (
    <div style={{
      maxWidth: 620, margin: '0 auto', padding: '0 20px',
      display: 'flex', flexDirection: 'column', height: 'calc(100vh - 56px)',
    }}>
      {/* Header */}
      <div style={{ padding: '16px 0 12px', flexShrink: 0, borderBottom: '2.5px solid var(--ink)' }}>
      ...
      {/* Input */}
      <div style={{ flexShrink: 0, borderTop: '2.5px solid var(--ink)', padding: '12px 0 16px' }}>
